    ...
    <header>
        <table style="width: 100%; border:1px solid black;border-radius: 5px;padding:10px !important" class="">
            <tr style="">
                ...
                <td style="">
                   : {{!empty($member_info->no_of_share) ? $member_info->no_of_share:''}}
                </td>
            </tr>
            <tr style="">
                <td style="">
                ...
                <td style="">
                   : {{!empty($member_info->Uid) ? $member_info->Uid:''}}
                </td>
                <td style="">
                    <strong>Share Amount  </strong>
                </td>
                <td style="">
                   : {{!empty($member_info->share_amount) ? $member_info->share_amount:''}}
                </td>
            </tr>
        </table>
    </header>
        ...
        <table class="list_table" style="width: 100%;" >
            ...
            @php 
                ...
                @if (!empty($payment_list))
                    ...
                    @php 
                    $total_amt +=$value->share_amount * $value->no_of_share ;
                @endphp
                        <tr>
                            ...
                            <td style="text-align:center">{{ $value->invoice_code }}</td>
                            <!-- <td style="text-align:center">{{ $value->member_code_no }}</td> -->
                            <!-- <td>{{ $value->member_name }}</td> -->
                            <td>{{ $value->from_month }}</td>
                            <td>{{ $value->year }}</td>
                            <td style="text-align:center">{{ $value->no_of_share }}</td>
                            <td style="text-align:right">{{ $value->share_amount }}</td>
                            <td style="text-align:right">{{ $value->share_amount * $value->no_of_share}}</td>
            ...
            <tr>
                    <tr>
                        <td style="text-align:right;  font-family: Monospace, sans-serif;" colspan="7"><strong> Total =</strong></td>   
                        <td style="text-align:right;  font-family: Monospace, sans-serif;" ><strong> {{$total_amt}}</strong></td>
                    </tr>
                    </tr>
            </tfoot>
        </table>
