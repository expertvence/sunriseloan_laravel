...
<div class="card-body"
    style="background: linear-gradient(135deg,#f8fbff,#eef3f9); border-radius: 28px; padding: 2.5rem; box-shadow: 0 25px 50px -12px rgba(0,20,40,0.18); width: 100%; margin: 0 auto; position: relative; overflow: hidden;">

    <!-- Decorative Background Glow -->
    <div
        style="position:absolute; top:-80px; right:-80px; width:200px; height:200px; background:radial-gradient(circle,#0a2540 0%,transparent 70%); opacity:0.08;">
    </div>

    <!-- USER PROFILE HEADER -->
    <div
        style="display:flex; align-items:center; gap:18px; margin-bottom:32px; border-bottom:1px solid #e6edf5; padding-bottom:22px;">

        <!-- USER IMAGE / ICON -->
        <div
            style="width:72px; height:72px; border-radius:20px; overflow:hidden; background:linear-gradient(145deg,#0a2540,#1f3a5f); display:flex; align-items:center; justify-content:center; color:white; font-size:32px; box-shadow:0 8px 20px rgba(0,0,0,0.15);">
        ...
        <div> <button onclick="goBack()"
                style="margin-bottom:15px; background:#0a2540; color:white; border:none;
padding:10px 18px; border-radius:12px; cursor:pointer;">
    ...
    <!-- FORM START -->
    <form id="passwordForm" action="{{ route('manager-save-password') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Old Password -->
        <div style="position:relative;margin-bottom:22px;">
            <i class="fas fa-lock"
                style="position:absolute; left:16px; top:50%; transform:translateY(-50%); color:#8a9bb5;"></i>

            <input type="password" name="old_password" id="old_password"
                style="width:100%; padding:14px 50px 14px 48px; border:1.8px solid #e2e8f0; border-radius:16px;"
                placeholder="Enter current password" required>

            <i class="fas fa-eye" onclick="togglePassword('old_password',this)"
